...
class PositEncode(val POSIT_WIDTH: Int, val VECTOR_SIZE: Int) extends Module {
  ...
  val regime_init  = Wire(UInt(POSIT_WIDTH.W - 1.W))
  val regime       = Wire(Vec(VECTOR_SIZE, UInt(POSIT_WIDTH.W - 1.W)))
  ...
  for(i <- 0 until VECTOR_SIZE){
    regime_width(i) := Mux(k_sign(i) === 1.U, ((~regime_k+1) + 1), regime_k + 2)
  }
  ...
  //进行右移操作(先左移再右移)
  var MAX_SHIFT   = FRAC_WIDTH.U + es.U + 1.U
  var SHIFT_WIDTH = log2Ceil(MAX_SHIFT)
  val shift = Wire(Vec(VECTOR_SIZE, UInt(SHIFT_WIDTH.W)))

  for(i <- 0 until VECTOR_SIZE){
    shift(i) := Mux(regime_width(i) >= POSIT_WIDTH.U, MAX_SHIFT, regime_width(i) + FRAC_WIDTH.U + es.U  - POSIT_WIDTH.U + 1.U)
  }
  ...
  for(i <- 0 until VECTOR_SIZE){
    value_before_shift(i) := rg_es_frac(i) << MAX_SHIFT
  ...
  for(i <- 0 until VECTOR_SIZE){
   Cat(value_after_shift(i), round_bits(i)) := value_after_shift(i)(MAX_SHIFT + POSIT_WIDTH - 2, 0)
