...
class CompTree(val N: Int, val WIDTH: Int) extends Module {
  val io = IO(new Bundle {
    val operands_i = Input(Vec(N, SInt(WIDTH.W))) // N 个有符号输入
    val result_o   = Output(SInt(WIDTH.W))        // 最大值的输出
  })
  ...
  if (N == 1) {
    ...
    // 递归实例化 CompTree 模块
    val compTreeA = Module(new CompTree(N_A, WIDTH))
    compTreeA.io.operands_i := operands_i_A

    val compTreeB = Module(new CompTree(N_B, WIDTH))
