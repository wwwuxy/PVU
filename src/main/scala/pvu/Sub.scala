...
class Sub(val POSIT_WIDTH: Int, val VECTOR_SIZE: Int, val ALIGN_WIDTH: Int, val ES: Int) extends Module { // # Posit向量减法单元
  ...
  for (i <- 0 until VECTOR_SIZE) {
    ...
    when (same_sign) { // # 同号执行减法
      val mant1 = io.pir_frac1_aligned(i)
      val mant2 = io.pir_frac2_aligned(i)
      val mant1_greater = mant1 > mant2
      
      io.pir_frac_o(i) := Mux(mant1_greater, mant1 - mant2, mant2 - mant1) // # 计算尾数差值
      io.pir_sign_o(i) := Mux(io.pir_sign1_i(i) === 1.U, 
                               Mux(mant1_greater, 1.U, 0.U), 
                               Mux(mant1_greater, 0.U, 1.U)) // # 计算结果符号位
      
      io.pir_exp_o(i) := io.pir_exp1_i(i) // # 减法不改变指数
      io.overflow(i) := 0.U
      ...
      io.overflow(i) := carry
      io.frac_truncate(i) := (carry && sum(ALIGN_WIDTH-1, 0).orR).asUInt
    }
  }
}