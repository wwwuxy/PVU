...
class Div(val POSIT_WIDTH: Int, val VECTOR_SIZE: Int, val ALIGN_WIDTH: Int, val ES:Int) extends Module {
  ...
  for (i <- 0 until VECTOR_SIZE) {
    ...
    val divider = Module(new IntDivider(FRAC_WIDTH+1))
    divider.io.dividend := io.pir_frac1_i(i)
    divider.io.divisor := io.pir_frac2_i(i)
    io.pir_frac_o(i) := divider.io.quotient
