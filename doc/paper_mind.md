...
postlet vmul(long* a, long* b, long* result, int len) {
    ...
    __asm__ (
        ...
        "    vmul.vv v2, v0, v1      \n"  // v2 = v0 * v1
        "    vsw.v v2, (%3)          \n"  // 将结果 v2 存储到结果向量中
        "    addi %1, %1, 16         \n"  // 更新向量 a 的指针，32位元素4个字节
        "    addi %2, %2, 16         \n"  // 更新向量 b 的指针，32位元素4个字节
        "    addi %3, %3, 16         \n"  // 更新结果向量的指针，32位元素4个字节
        "    addi %0, %0, -4         \n"  // 递减剩余元素数量
        "    bnez %0, 1b             \n"  // 如果还有元素，继续循环
        : // 无输出
        : "r"(length), "r"(p1), "r"(p2), "r"(res) // 输入操作数
        : "v0", "v1", "v2", "t0"  // 受影响的寄存器
    );
...
本文余下部分的组织结构如下：第二部分介绍了Posit数制系统和RISC-V指令集。第三部分对比了本研究和其他现有研究的异同。第四部分详细阐述了PVU的逻辑实现，重点介绍了乘法模块。第五部分描述了自定义的指令集架构扩展和编译支持。第六部分描述了硬件成本和运算精度。最后一部分讲述了总结与展望。
...
在‌指令集体系结构层面，我们通过RISC-V的V扩展，为PVU设计了运算所必须的向量指令。如表2所示，
